 ...
 * `DriverProfitDatabase.VERSION` e a presença da tabela `vehicles`. Um
...
class ImportBackupUseCase(
    ...
    private val database: DriverProfitDatabase,
) {
    suspend operator fun invoke(source: Uri): ImportBackupResult =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                database.close()
                val target = context.getDatabasePath(DriverProfitDatabase.NAME)
    ...
    private fun validate(file: File): BackupError? = try {
        SQLiteDatabase.openDatabase(file.path, null, SQLiteDatabase.OPEN_READONLY).use {
            ...
            if (fileVersion > DriverProfitDatabase.VERSION) return@use BackupError.NEWER_APP_VERSION
